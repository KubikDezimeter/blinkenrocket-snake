        ...
        perSystem = { config, self', inputs',  pkgs, pkgsCross, system, ... }: rec {
            ...
            packages.default = pkgs.stdenv.mkDerivation {
                ...
                installPhase = ''
                    runHook preInstall
                    mkdir -p $out
                    cp build/main.hex $out/main.hex
                    runHook postInstall
                '';
            };

            packages.avr-pico-programmer = pkgs.buildNpmPackage rec {
                name = "avr-pico-programmer";
                ...
                src = pkgs.fetchFromGitHub {
                  owner = "SpeedyCraftah";
                  repo = "avr-pico-programmer";
                  rev = "017911dde0969e88af3aba8b0477bcd8497cefe2";
                  hash = "sha256-DdU5weE+pQp+0VUU4SUZJX9FroR9onpxIFDoT1kBid0=";
                };

                sourceRoot = "${src.name}/client";

                npmDepsHash = "sha256-orrSPPaBjpDgJPIybdCJliex7XlmDbAl8VsT8wX0dfI=";

                dontNpmBuild = true;

                postPatch = ''
                    substituteInPlace program.js --replace-fail 'COM3' '/dev/ttyACM0'
                    substituteInPlace program.js --replace-fail ' 5000' ' 9999999'

                    sed -i '/const data = /i \
                        if (process.argv.length !== 3) {\
                            console.log("Usage: " + process.argv[1] + " <firmware.hex>");\
                            process.exit(1);\
                        }\
                        \
                        ' program.js
                    substituteInPlace program.js --replace-fail '"./fw.bin"' 'process.argv[2]'

                    jq '. + {"bin": {"avr-pico-programmer": "program.js"}}' package.json > package.json.tmp && mv package.json.tmp package.json
